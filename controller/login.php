...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($user['user_type'] !== 'authorized') {
            // User type is not authorized, deny login
            echo "Access denied! Only authorized users can log in.";
            header('Location: ../login.php?error=Access Denied! Only authorized users can log in.');
            exit();
        }
